...
export const claimCoupon = async (req: Request, res: Response): Promise<void> => {
    ...
    //  Check if user has already claimed ANY coupon (checking full array)
    ...
    if (existingClaim) {
      res.status(429).json({ message: "You have already claimed a coupon and cannot claim another." });
      return;
    }
